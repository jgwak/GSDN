"""Source: https://raw.githubusercontent.com/griegler/octnet/master/example/00_create_data/vis.py
"""
...
def write_ply_boxes(out_path, bxs, binary=False):
  ...
  if binary:
    def write_fcn(x, y, z, c0, c1, c2): return f.write(
        struct.pack('<fffBBB', x, y, z, c0, c1, c2))
  else:
    def write_fcn(x, y, z, c0, c1, c2): return f.write(
        '%f %f %f %d %d %d\n' % (x, y, z, c0, c1, c2))
  for box in bxs:
  ...
  if binary:
    def write_fcn(i0, i1, i2): return f.write(
        struct.pack('<Biii', 3, i0, i1, i2))
  else:
    def write_fcn(i0, i1, i2): return f.write('3 %d %d %d\n' % (i0, i1, i2))
  vidx = 0
  for box in bxs:
  ...
  for d in range(grid.shape[0]):
    for h in range(grid.shape[1]):
      for w in range(grid.shape[2]):
        ...
        bxs.append((x, x+width, y, y+width, z, z +
