...
class InstanceAPCalculator:
  ...
  def evaluate(self):
    matches = {}
    for i, (pred, gt) in enumerate(zip(self.preds, self.gts)):
      # assign gt to predictions
      gt2pred, pred2gt = self.assign_instances_for_scan(i, pred, gt)
      matches[i] = {}
      matches[i]['gt'] = gt2pred
      matches[i]['pred'] = pred2gt
    ap_scores = self.evaluate_matches(matches)
    return self.compute_averages(ap_scores)

  @staticmethod
  def get_instances(gts):
    instances = {}
    for label in CLASS_LABELS:
      instances[label] = []
    for gt_idx, (gt_mask, gt_class_idx) in enumerate(gts):
      label_id = gt_class_idx - 2
      instances[CLASS_LABELS[label_id]].append({
          'instance_id': gt_idx,
          'label_id': label_id,
          'vert_count': gt_mask.sum(),
          'gt_mask': gt_mask,
          'med_dist': -1,
          'dist_conf': 0.0,
      })
  ...
  def assign_instances_for_scan(self, fileidx, preds, gts):
    gt_instances = self.get_instances(gts)
    # get gt instances
    # associate
    ...
    if len(gts) == 0 and len(preds) != 0:
      bool_void = np.full(preds[0][0].size, True, dtype=bool)
    else:
      bool_void = np.sum(np.array([gt_mask for gt_mask, _ in gts]), 0) == 0
    # go thru all prediction masks
    for pred_id, (pred_mask, label_id, conf) in enumerate(preds):
      label_name = CLASS_LABELS[label_id - 2]
      # read the mask
      assert pred_mask.size == bool_void.size
      # convert to binary
      ...
      pred_instance = {}
      pred_instance['filename'] = f'{fileidx}{pred_id}'
      pred_instance['pred_id'] = num_pred_instances
      pred_instance['label_id'] = label_id - 2
      ...
      for (gt_num, gt_inst) in enumerate(gt2pred[label_name]):
        intersection = np.count_nonzero(np.logical_and(gt_inst['gt_mask'], pred_mask))
        if intersection > 0:
    ...
    for di, (min_region_size, distance_thresh,
      ...
      for oi, overlap_th in enumerate(OVERLAPS):
        ...
        for li, label_name in enumerate(CLASS_LABELS):
          ...
          if has_gt and has_pred and y_true.size:
